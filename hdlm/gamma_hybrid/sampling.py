...
from hdlm.model import utils as mautils
...
class EulerPredictor(Predictor):
    def update_fn(self, score_fn, x, positions, t, step_size, attn_mask):
        ...
        x_updated = self.graph.sample_rate(x, rev_rate)
        return x_updated
    ...
    def pc_sampler(model):
        ...
        for current_step in range(1, metaschedule.max_step(seq_len)):
            ...
            t, attn_mask, tokens_to_denoise, active_mask, settled_mask, _ = msutils.compute_t_and_attn(
                batch_size, seq_len, metaschedule, current_step, annealing.attention.context_type, annealing.attention.block_type, device, eps)
            
            step = torch.ones_like(t) * step_size
            step[tokens_to_denoise] = 0.99 * active_eps
            step[settled_mask] = 0.99 * eps
            t = t.unsqueeze(0).expand(batch_size, -1)
            step = step.unsqueeze(0).expand(batch_size, -1)
            step = torch.clamp(step, min=0.99 * eps, max=step_size)

            tokens_to_denoise = tokens_to_denoise.unsqueeze(0).expand(batch_size, seq_len)
